	...
	const struct argconfig_commandline_options *opts)
{
	...
	printf("RETURN VALUE OF argsconfig_parse: %d\n", ret);
	fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 84, \"func\": \"parse_and_open\", \"msg\": \"RETURN VALUE OF argsconfig_parse: %d\"}\n", ret);
	if (ret) {
		printf("Returning argsconfig_parse: %d\n", ret);
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 72, \"func\": \"parse_and_open\", \"branch_name\": \"ret\", \"state\": true, \"msg\": \"Returning argsconfig_parse: %d\"}\n", ret);
		return ret;
	}

	ret = get_dev(argc, argv, O_RDONLY);
	printf("RETURN VALUE OF get_dev: %d\n", ret);
	fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 85, \"func\": \"parse_and_open\", \"msg\": \"RETURN VALUE OF get_dev: %d\"}\n", ret);
	if (ret < 0) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 73, \"func\": \"parse_and_open\", \"branch_name\": \"ret < 0\", \"state\": true}\n");
		...
		struct command *cmd, struct plugin *plugin)
{
	...
	if (cfg.action == NVME_PEVENT_LOG_RELEASE_CTX) {
		...
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 86, \"func\": \"get_persistent_event_log\", \"branch_name\": \"cfg.action == NVME_PEVENT_LOG_RELEASE_CTX\", \"state\": true, \"msg\": \"Releasing Persistent Event Log Context\"}\n");
		goto free_pevent;
	}
	...
	if (!cfg.log_len && cfg.action != NVME_PEVENT_LOG_EST_CTX_AND_READ) {
		...
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 87, \"func\": \"get_persistent_event_log\", \"branch_name\": \"!cfg.log_len && cfg.action == NVME_PEVENT_LOG_EST_CTX_AND_READ\", \"state\": true, \"msg\": \"Establishing Persistent Event Log Context\"}\n");
		goto free_pevent;
	}
	...
	if (!err) {
		...
		if (pevent_collected->gen_number != pevent->gen_number) {
			...
			fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 88, \"func\": \"get_persistent_event_log\", \"branch_name\": \"pevent_collected->gen_number != pevent->gen_number\", \"state\": true, \"msg\": \"Collected Persistent Event Log may be invalid, Re-read the log is reiquired\"}\n");
			goto free;
		}
...
static int get_boot_part_log(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	if (err != bpsz) {
		...
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 89, \"func\": \"get_boot_part_log\", \"branch_name\": \"err != bpsz\", \"state\": false, \"msg\": \"Data flushed into file %s\"}\n", cfg.file_name);
	}
	err = 0;
...
static int id_iocs(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	if (!err) {
		...
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 90, \"func\": \"id_iocs\", \"branch_name\": \"!err\", \"state\": true, \"msg\": \"NVMe Identify I/O Command Set:\"}\n");
...
static int id_domain(int argc, char **argv, struct command *cmd, struct plugin *plugin) {
	...
	if (!err) {
		...
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 74, \"func\": \"id_domain\", \"branch_name\": \"!err\", \"state\": true, \"result\": \"success\", \"msg\": \"NVMe Identify command for Domain List is successful:\nNVMe Identify Domain List:\"}\n");
...
static int virtual_mgmt(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	if (!err) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 75, \"func\": \"virtual_mgmt\", \"branch_name\": \"!err\", \"state\": true, \"result\": \"success\"}\n");
...
static int fw_download(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	if (!err) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 76, \"func\": \"fw_download\", \"branch_name\": \"!err\", \"state\": true, \"result\": \"success\"}\n");
...
static int dsm(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	if (err < 0)
		...
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 77, \"func\": \"dsm\", \"branch_name\": \"err < 0 || err != 0\", \"state\": false, \"result\": \"success\"}\n");
...
static int copy(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	if (err < 0)
		...
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 78, \"func\": \"copy\", \"branch_name\": \"err < 0 || err != 0\", \"state\": false, \"result\": \"success\"}\n");
...
static int flush(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	if (err < 0)
		...
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 79, \"func\": \"flush\", \"branch_name\": \"err < 0 || err != 0\", \"state\": false, \"result\": \"success\", \"msg\": \"NVMe Flush: success\"}\n");
	}
close_fd:
	close(fd);
ret:
...
static int resv_acquire(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	if (err < 0)
		...
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 80, \"func\": \"resv_acquire\", \"branch_name\": \"err < 0 || err != 0\", \"state\": false, \"result\": \"success\"}\n");
...
static int resv_register(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	if (err < 0)
		...
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 81, \"func\": \"resv_register\", \"branch_name\": \"err < 0 || err != 0\", \"state\": false, \"result\": \"success\", \"msg\": \"NVME Reservation success\"}\n");
...
static int resv_release(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	if (err < 0)
		...
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 82, \"func\": \"resv_release\", \"branch_name\": \"err < 0 || err != 0\", \"state\": false, \"result\": \"success\"}\n");
...
static int submit_io(int opcode, char *command, const char *desc,
		     int argc, char **argv)
{
	fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 5, \"func\": \"submit_io\", \"branch_name\": \"\", \"state\": null}\n");
	...
	if (opcode != nvme_cmd_write) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 6, \"func\": \"submit_io\", \"branch_name\": \"opcod != nvme_cmd_write\", \"state\": true}\n");
		err = fd = parse_and_open(argc, argv, desc, opts);
		if (fd < 0) {
			fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 8, \"func\": \"submit_io\", \"branch_name\": \"fd < 0\", \"state\": true}\n");
			goto ret;
		}
	} else {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 7, \"func\": \"submit_io\", \"branch_name\": \"opcod != nvme_cmd_write\", \"state\": false}\n");
		err = argconfig_parse(argc, argv, desc, opts);
		if (err) {
			fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 9, \"func\": \"submit_io\", \"branch_name\": \"err\", \"state\": true}\n");
			goto ret;
		}
		err = fd = open_exclusive(argc, argv, cfg.force);
		if (err < 0) {
			fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 10, \"func\": \"submit_io\", \"branch_name\": \"err < 0\", \"state\": true}\n");
			if (errno == EBUSY) {
				fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 11, \"func\": \"submit_io\", \"branch_name\": \"errno == EBUSY\", \"state\": true}\n");
				fprintf(stderr, "Failed to open %s.\n",
					basename(argv[optind]));
				fprintf(stderr,
					"Namespace is currently busy.\n");
				if (!cfg.force) {
					fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 12, \"func\": \"submit_io\", \"branch_name\": \"!cfg.force\", \"state\": true}\n");
					fprintf(stderr,
					"Use the force [--force] option to ignore that.\n");
				}
			} else {
				fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 13, \"func\": \"submit_io\", \"branch_name\": \"errno == EBUSY\", \"state\": false}\n");
				argconfig_print_help(desc, opts);
			}
			goto ret;
		}
	...
	if (!cfg.namespace_id) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 14, \"func\": \"submit_io\", \"branch_name\": \"!cfg.namespace_id\", \"state\": true}\n");
		err = nvme_get_nsid(fd, &cfg.namespace_id);
		if (err < 0) {
			fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 15, \"func\": \"submit_io\", \"branch_name\": \"err < 0\", \"state\": true}\n");
			fprintf(stderr, "get-namespace-id: %s\n", nvme_strerror(errno));
			goto close_fd;
		}
	}
	...
	if (cfg.prinfo > 0xf) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 16, \"func\": \"submit_io\", \"branch_name\": \"cfg.prinfo > 0xf\", \"state\": true}\n");
		err = -EINVAL;
		goto close_fd;
	}
	...
	control |= (cfg.prinfo << 10);
	if (cfg.limited_retry) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 17, \"func\": \"submit_io\", \"branch_name\": \"cfg.limited_retry\", \"state\": true}\n");
		control |= NVME_IO_LR;
	}
	if (cfg.force_unit_access) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 18, \"func\": \"submit_io\", \"branch_name\": \"cfg.force_unit_access\", \"state\": true}\n");
		control |= NVME_IO_FUA;
	}
	if (cfg.storage_tag_check) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 19, \"func\": \"submit_io\", \"branch_name\": \"cfg.storage_tag_check\", \"state\": true}\n");
		control |= NVME_IO_STC;
	}
	if (cfg.dtype) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 20, \"func\": \"submit_io\", \"branch_name\": \"cfg.dtype\", \"state\": true}\n");
		if (cfg.dtype > 0xf) {
			fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 21, \"func\": \"submit_io\", \"branch_name\": \"cfg.dtype > 0xf\", \"state\": true}\n");
	...
	if (strlen(cfg.data)) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 22, \"func\": \"submit_io\", \"branch_name\": \"strlen(cfg.data)\", \"state\": true}\n");
		dfd = open(cfg.data, flags, mode);
		if (dfd < 0) {
			fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 23, \"func\": \"submit_io\", \"branch_name\": \"dfd < 0\", \"state\": true}\n");
			perror(cfg.data);
			err = -EINVAL;
			goto close_fd;
		}
	...
	if (strlen(cfg.metadata)) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 24, \"func\": \"submit_io\", \"branch_name\": \"strlen(cfg.metadata)\", \"state\": true}\n");
		mfd = open(cfg.metadata, flags, mode);
		if (mfd < 0) {
			fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 25, \"func\": \"submit_io\", \"branch_name\": \"mfd < 0\", \"state\": true}\n");
			perror(cfg.metadata);
			err = -EINVAL;
			goto close_dfd;
		}
	...
	if (!cfg.data_size)	{
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 26, \"func\": \"submit_io\", \"branch_name\": \"!cfg.data_size\", \"state\": true, \"msg\": \"data size not provided\"}\n");
	...
	if (nvme_get_logical_block_size(fd, cfg.namespace_id,
		...
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 27, \"func\": \"submit_io\", \"branch_name\": \"nvme_get_logical_block_size > 0\", \"state\": true}\n");
			goto close_mfd;
	}				

	buffer_size = ((long long)cfg.block_count + 1) * logical_block_size;
	if (cfg.data_size < buffer_size) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 28, \"func\": \"submit_io\", \"branch_name\": \"cfg.data_size < buffer_size\", \"state\": true, \"msg\": \"Rounding data size to fit block count (%lld bytes)\"}\n", buffer_size);
		fprintf(stderr, "Rounding data size to fit block count (%lld bytes)\n",
				buffer_size);
	} else {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 29, \"func\": \"submit_io\", \"branch_name\": \"cfg.data_size < buffer_size\", \"state\": false}\n");
		buffer_size = cfg.data_size;
	}

	buffer = nvme_alloc(buffer_size, &huge);
	if (!buffer) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 30, \"func\": \"submit_io\", \"branch_name\": \"!buffer\", \"state\": true}\n");
		err = -ENOMEM;
		goto close_mfd;
	}

	if (cfg.metadata_size) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 31, \"func\": \"submit_io\", \"branch_name\": \"cfg.metadata_size\", \"state\": true}\n");
		err = nvme_identify_ns(fd, cfg.namespace_id, &ns);
		if (err > 0) {
			fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 32, \"func\": \"submit_io\", \"branch_name\": \"err > 0\", \"state\": true}\n");
			nvme_show_status(err);
			goto free_buffer;
		} else if (err < 0) {
			fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 33, \"func\": \"submit_io\", \"branch_name\": \"err < 0\", \"state\": true}\n");
			fprintf(stderr, "identify namespace: %s\n", nvme_strerror(errno));
			goto free_buffer;
		}
		...
		if (!err) {
			fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 34, \"func\": \"submit_io\", \"branch_name\": \"!err\", \"state\": true}\n");
			sts = nvm_ns.elbaf[lba_index] & NVME_NVM_ELBAF_STS_MASK;
			pif = (nvm_ns.elbaf[lba_index] & NVME_NVM_ELBAF_PIF_MASK) >> 7; 
		}
		...
		if (ms && cfg.metadata_size < mbuffer_size) {
			fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 35, \"func\": \"submit_io\", \"branch_name\": \"ms && cfg.metadata_size < mbuffer_size\", \"state\": true}\n");
			fprintf(stderr, "Rounding metadata size to fit block count (%lld bytes)\n",
					mbuffer_size);
		} else {
			fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 36, \"func\": \"submit_io\", \"branch_name\": \"ms && cfg.metadata_size < mbuffer_size\", \"state\": false}\n");
			mbuffer_size = cfg.metadata_size;
		}
		mbuffer = malloc(mbuffer_size);
		if (!mbuffer) {
			fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 37, \"func\": \"submit_io\", \"branch_name\": \"!mbuffer\", \"state\": true}\n");
			err = -ENOMEM;
			goto free_buffer;
		}
	...
	if (invalid_tags(cfg.storage_tag, cfg.ref_tag, sts, pif)) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 38, \"func\": \"submit_io\", \"branch_name\": \"invalid_tags\", \"state\": true}\n");
		err = -EINVAL;
		goto free_buffer;
	}

	if ((opcode & 1)) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 39, \"func\": \"submit_io\", \"branch_name\": \"opcode & 1\", \"state\": true}\n");
		err = read(dfd, (void *)buffer, cfg.data_size);
		if (err < 0) {
			fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 40, \"func\": \"submit_io\", \"branch_name\": \"err < 0\", \"state\": true}\n");
			err = -errno;
			fprintf(stderr, "failed to read data buffer from input"
					" file %s\n", strerror(errno));
			goto free_mbuffer;
	...
	if ((opcode & 1) && cfg.metadata_size) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 41, \"func\": \"submit_io\", \"branch_name\": \"(opcode & 1) && cfg.metadata_size\", \"state\": true}\n");
		err = read(mfd, (void *)mbuffer, mbuffer_size);
		if (err < 0) {
			fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 42, \"func\": \"submit_io\", \"branch_name\": \"err < 0\", \"state\": true}\n");
			err = -errno;
	...
	if (cfg.show || cfg.dry_run) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 43, \"func\": \"submit_io\", \"branch_name\": \"cfg.show || cfg.dry_run\", \"state\": true}\n");
	...
	if (cfg.dry_run) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 44, \"func\": \"submit_io\", \"branch_name\": \"cfg.dry_run\", \"state\": true}\n");
		goto free_mbuffer;
	}
	...
	if (opcode == nvme_cmd_compare) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 45, \"func\": \"submit_io\", \"branch_name\": \"opcode == nvme_cmd_compare\", \"state\": true}\n");
		err = nvme_compare(&args);
	}
	else if (opcode & 1) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 46, \"func\": \"submit_io\", \"branch_name\": \"opcode & 1\", \"state\": true}\n");
		err = nvme_write(&args);
	}
	else {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 47, \"func\": \"submit_io\", \"branch_name\": \"(opcode == nvme_cmd_compare) || (opcode & 1)\", \"state\": false}\n");
		err = nvme_read(&args);
	}
	gettimeofday(&end_time, NULL);
	if (cfg.latency) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 48, \"func\": \"submit_io\", \"branch_name\": \"cfg.latency\", \"state\": true}\n");
		printf(" latency: %s: %llu us\n",
			command, elapsed_utime(start_time, end_time));
	}
	if (err < 0) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 49, \"func\": \"submit_io\", \"branch_name\": \"err < 0\", \"state\": true, \"msg\": \"submit-io: %s\"}\n", nvme_strerror(errno));
		fprintf(stderr, "submit-io: %s\n", nvme_strerror(errno));
		printf("submit-io: %s\n", nvme_strerror(errno));
	}
	else if (err) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 50, \"func\": \"submit_io\", \"branch_name\": \"err\", \"state\": true}\n");
		nvme_show_status(err);
	}
	else {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 51, \"func\": \"submit_io\", \"branch_name\": \"(err < 0) || err\", \"state\": false}\n");
		if (!(opcode & 1) && write(dfd, (void *)buffer, cfg.data_size) < 0) {
			fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 52, \"func\": \"submit_io\", \"branch_name\": \"!(opcode & 1) && write(dfd, (void *)buffer, cfg.data_size) < 0\", \"state\": true}\n");
			fprintf(stderr, "write: %s: failed to write buffer to output file\n",
					strerror(errno));
			err = -EINVAL;
		} else if (!(opcode & 1) && cfg.metadata_size &&
				write(mfd, (void *)mbuffer, mbuffer_size) < 0) {
			fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 53, \"func\": \"submit_io\", \"branch_name\": \"!(opcode & 1) && cfg.metadata_size && write(mfd, (void *)mbuffer, mbuffer_size) < 0\", \"state\": true}\n");		
			fprintf(stderr, "write: %s: failed to write meta-data buffer to output file\n",
					strerror(errno));
			err = -EINVAL;
		} else {
			fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 54, \"func\": \"submit_io\", \"branch_name\": \"!(opcode & 1) && etc\", \"state\": false, \"result\": \"success\", \"msg\": \"%s: Success\"}\n", command);
...
close_mfd:
	if (strlen(cfg.metadata)){
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 55, \"func\": \"submit_io\", \"branch_name\": \"strlen\", \"state\": true}\n");
		close(mfd);
	}
close_dfd:
	close(dfd);
...
{
	fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 4, \"func\": \"read_cmd\", \"branch_name\": \"\", \"state\": null}\n");
...
{
	fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 56, \"func\": \"write_cmd\", \"branch_name\": \"\", \"state\": null}\n");
...
{
	fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 57, \"func\": \"verify_cmd\", \"branch_name\": \"\", \"state\": null}\n");
	int err, fd;
	__u16 control = 0;
	__u8 lba_index, sts = 0, pif = 0;
	struct nvme_id_ns ns;
	...
	err = fd = parse_and_open(argc, argv, desc, opts);
	if (fd < 0) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 58, \"func\": \"verify_cmd\", \"branch_name\": \"fd < 0\", \"state\": true}\n");
		goto ret;
	}
	if (cfg.prinfo > 0xf) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 59, \"func\": \"verify_cmd\", \"branch_name\": \"cfg.prinfo < 0xf\", \"state\": true}\n");
		err = EINVAL;
		goto close_fd;
	}
	...
	control |= (cfg.prinfo << 10);
	if (cfg.limited_retry) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 60, \"func\": \"verify_cmd\", \"branch_name\": \"cfg.limited_retry\", \"state\": true}\n");
		control |= NVME_IO_LR;
	}
	if (cfg.force_unit_access) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 61, \"func\": \"verify_cmd\", \"branch_name\": \"cfg.force_unit_access\", \"state\": true}\n");
		control |= NVME_IO_FUA;
	}
	if (cfg.storage_tag_check) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 62, \"func\": \"verify_cmd\", \"branch_name\": \"cfg.storage_tag_check\", \"state\": true}\n");
		control |= NVME_IO_STC;
	}
	if (!cfg.namespace_id) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 63, \"func\": \"verify_cmd\", \"branch_name\": \"!cfg.namespace_id\", \"state\": true}\n");
		err = nvme_get_nsid(fd, &cfg.namespace_id);
		if (err < 0) {
			fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 64, \"func\": \"verify_cmd\", \"branch_name\": \"err < 0\", \"state\": true}\n");
			fprintf(stderr, "get-namespace-id: %s\n", nvme_strerror(errno));
			goto close_fd;
		}
	}
	...
	err = nvme_identify_ns(fd, cfg.namespace_id, &ns);
	if (err) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 65, \"func\": \"verify_cmd\", \"branch_name\": \"err\", \"state\": true}\n");
		nvme_show_status(err);
		goto close_fd;
	} else if (err < 0) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 66, \"func\": \"verify_cmd\", \"branch_name\": \"err < 0\", \"state\": true}\n");
		fprintf(stderr, "identify namespace: %s\n", nvme_strerror(errno));
		goto close_fd;
	}
	...
	err = nvme_identify_ns_csi(fd, cfg.namespace_id, 0, NVME_CSI_NVM, &nvm_ns);
	if (!err) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 67, \"func\": \"verify_cmd\", \"branch_name\": \"!err\", \"state\": true}\n");
		nvme_id_ns_flbas_to_lbaf_inuse(ns.flbas, &lba_index);
		sts = nvm_ns.elbaf[lba_index] & NVME_NVM_ELBAF_STS_MASK;
		pif = (nvm_ns.elbaf[lba_index] & NVME_NVM_ELBAF_PIF_MASK) >> 7;
	}
	...
	if (invalid_tags(cfg.storage_tag, cfg.ref_tag, sts, pif)) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 68, \"func\": \"verify_cmd\", \"branch_name\": \"invalid_tags\", \"state\": true}\n");
		err = -EINVAL;
		goto close_fd;
	}
	...
	if (err < 0) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 69, \"func\": \"verify_cmd\", \"branch_name\": \"err < 0\", \"state\": true}\n");
		fprintf(stderr, "verify: %s\n", nvme_strerror(errno));
	}
	else if (err != 0) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 70, \"func\": \"verify_cmd\", \"branch_name\": \"err != 0\", \"state\": true}\n");
		nvme_show_status(err);
	}
	else {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 71, \"func\": \"verify_cmd\", \"branch_name\": \"err < 0 || err != 0\", \"state\": false}\n");
...
int main(int argc, char **argv)
{
	...
	for (i = 1; i < argc; i++) {
		strcat(param_string, " ");
		strcat(param_string, argv[i]);
	}

	fprintf(fptr, "{\"type\": \"start_signal\", \"param\": \"%s\" }\n", param_string);

	nvme.extensions->parent = &nvme;
	if (argc < 2) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 1, \"func\": \"main\", \"branch_name\": \"argc < 2\", \"state\": true}\n");
	...
	if (err == -ENOTTY) {
		fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 2, \"func\": \"main\", \"branch_cond\": \"err == -ENOTTY\", \"state\": true}\n");
		general_help(&builtin);
	}

	fprintf(fptr, "{\"type\": \"instrumentation\", \"id\": 3, \"func\": \"main\", \"branch_name\": \"argc < 2\", \"state\": false}\n");
	fclose(fptr);
	printf("ERR value: %d\n", err);
	return err ? 1 : 0;
}
